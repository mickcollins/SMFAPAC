...
@EXPORT = qw(
  ...
  top_title  pprint_nl new_line query pause popup_ask clear_screen
  );
#
# PerlMenu - Perl library module for curses-based menus & data-entry templates
#%PERL5ONLY% DO NOT REMOVE OR CHANGE THIS LINE
...
# Clear screen and put title at the top.
  ...
  &addstr($title); $m_col += length($title);
...
#**********
sub pprint_nl {
  local($text,$skip,$position_online) = @_;
  if ($position_online) { $m_row=$position_online; $m_col=0; &move($m_row,$m_col);  }
# Check for end-of-line wrap
  while ($m_col+length($text) > $COLS) {
...
# Prompt until we get a good answer.
  while (1) {
    &getyx($window,$m_row,$m_col);
    $ch = &menu_getstr($m_row,$m_col,"$prompt $sel_prompt ",0); &new_line(1);
    if ($ch ne "") {
      $ch = substr($ch,0,1);
      $ch =~ tr/A-Z/a-z/;
