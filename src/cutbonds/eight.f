...
c     dimension isix(natomm,natomm)
      dimension ieight(500,8),ifr(8)

      ieight=0

      nafrag=numat(nfrag)

      if(nafrag.lt.8)return
       ...
       do k3=1,itp(n1)+1
        ...
        n4=ibo(n1,k3)
         ...
         do k4=1,itp(n3)+1
          ...
          if(n5.eq.n4)go to 93
          if(ilink(n5,n1).eq.1)go to 93
          if(ilink(n5,n2).eq.1)go to 93
         ...
         do k6=1,itp(n5)+1
          ...
          if(ilink(n7,n6).eq.1)go to 95

         do k7=1,itp(n6)+1
          n8=ibo(n6,k7)
          if(n8.eq.n1)go to 96
          if(n8.eq.n2)go to 96
          if(n8.eq.n3)go to 96
          if(n8.eq.n4)go to 96
          if(n8.eq.n5)go to 96
          if(n8.eq.n7)go to 96
          if(ilink(n8,n1).eq.1)go to 96
          if(ilink(n8,n2).eq.1)go to 96
          if(ilink(n8,n3).eq.1)go to 96
          if(ilink(n8,n4).eq.1)go to 96
          if(ilink(n8,n5).eq.1)go to 96

         if(ilink(n8,n7).eq.1)then
...
c we have a 8 ring
           ...
           if(nring.gt.0)then
            do k=1,nring
             idiff=0
             do j=1,8
              idiff=idiff+iabs(ieight(k,j)-ifr(j))
             enddo
...
100   ncount=0
      do n=1,nafrag
       do i=1,nring
        isum=0
        do j=1,8
        isum=isum+ma(n,ieight(i,j))
        enddo
        if(isum.gt.5.and.isum.lt.8)then
         ncount=ncount+1
         do j=1,8
          ma(n,ieight(i,j))=1
          ma(ieight(i,j),n)=1
